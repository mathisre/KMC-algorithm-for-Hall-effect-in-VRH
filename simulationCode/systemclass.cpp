...
double angle(int x, int y){
    if (x == 0) return M_PI/2 * (y > 0) + 3*M_PI/2*(y < 0);
    if (y == 0) return 0*(x>0) + M_PI*(x<0);
    double xF = (double) x;
    double yF = (double) y;
    if (xF > 0 && yF > 0) return atan(yF/xF);
    if (xF < 0 && yF > 0) return M_PI + atan(yF/xF);
    if (xF < 0 && yF < 0) return M_PI + atan(yF/xF);
...
double ESystem::getOccnum() const
{
    return occnum;
}
