...
void run::runCurrent(class params *p) //2D only so far!!!
{
  ...
  CGcontrol *esc;
  MKcurrent *MKcurr;
  string st, proposedJumpsFilename, discardedJumpsFilename, performedJumpsFilename;

  double  Ex, Ey, Ez, energy,t, Hx, Hy, Hz; // mc,
  int L, D, N, steps, run, runs; //, im, i, s, f

  printf("\n-----Setting up measure system-----\n\n");
  ...
  es->reconfig(p->istate); //creates a new (random) initial stat
  es->setSPE(true);
...
//  printES(es);  // Prints map of initial state (electrons and empty sites)
  ...
  MKcurr->init(D, L, N, p->maxJL, p->loc, 1.0/p->temp,p->maxProbability, p->doTriJumps); // <- calculates rates
  MKcurr->setES(es);
...
//  fflush(stdout);
  ...
  for (run = 0; run < runs; run++)
  {
    ...
    printf("For fil\n");
    fflush(stdout);
//    MKcurr->printWeightedAreaTimeData();
    MKcurr->jumpsToFileSmall(p->outputprefix + "jumps_" + p->outputendfix + "_run_" + IntToStr(run) + ".dat",steps);



//    MKcurr->sampleProposed3SiteJumps(steps);
//    proposedJumpsFilename = p->outputprefix + "proposedJumps_" + p->outputendfix + "_run_" + IntToStr(run) + ".dat";
//    discardedJumpsFilename = p->outputprefix + "discardedJumps_" + p->outputendfix + "_run_" + IntToStr(run) + ".dat";
//    performedJumpsFilename = p->outputprefix + "performedJumps_" + p->outputendfix + "_run_" + IntToStr(run) + ".dat";

//    MKcurr->comparePerformedWithInitialized(steps);
//    MKcurr->sample3SiteJumpRates(steps, proposedJumpsFilename, discardedJumpsFilename, performedJumpsFilename);
//    MKcurr->sample2SiteJumpRates(steps);

//    MKcurr->writeAreaHistogramToFile(p->outputprefix + "area_histogram_" + p->outputendfix + ".dat");
...
// printES(es);
 ...
 MKcurr->setE(Ex, Ey, Ez);
 printf("E set to [%le %le %le]\n",Ex,Ey,Ez);
 MKcurr->init(D,L,N, p->maxJL, p->loc, 1.0/p->temp,p->maxProbability, p->doTriJumps);
 MKcurr->setES(es);
...
void run::runCurrentRejection(class params *p) //2D only so far!!!
{

  ESystem *es;
  CGcontrol *esc;
  MKcurrentRejection *MKcurr;
  string st;
...
//  printf("Energy: %le\n",energy);
  ...
  MKcurr->setE(Ex, Ey, Ez);
  MKcurr->setH(Hx, Hy, Hz);
  printf("Temperature set to %4.3f\n", p->temp);
  printf("E set to [%le %le %le]\n",Ex,Ey,Ez);
  printf("H set to [%le %le %le]\n",Hx,Hy,Hz);
  fflush(stdout);

  MKcurr->init(D, L, N, p->maxJL, p->loc, 1.0/p->temp,p->maxProbability); // <- calculates rates
  MKcurr->setES(es);
  MKcurr->setWritelines(p->writelines);
  MKcurr->setRatefun(p->ratefun); //
...
void run::runCurrentRejectionRandomLattice(class params *p) //2D only so far!!!
{
  ...
  printf("\n-----Setting up random sites rejection system-----\n\n");
...
//  printf("Energy: %le\n",energy);
  ...
  for (run = 0; run < runs; run++)
  {
    MKcurr->runCurrentRandomLattice(steps, energy, t);
    ...
    MKcurr->jumpsToFileSmallRandomLattice(p->outputprefix + "jumps_" + p->outputendfix + "_run_" + IntToStr(run) + ".dat",steps);
...
void run::runCurrentRejectionMaps(class params *p) //2D only so far!!!
{
//  setlocale(LC_ALL,"");
...
//  printf("Running current\n");
  ...
  printf("\n-----Setting up rejection maps system-----\n\n");
...
//  fflush(stdout);
  ...
  for (run = 0; run < runs; run++)
  {
    MKcurr->runCurrentMaps(steps, energy, t);
    ...
    MKcurr->jumpsToFileSmall(p->outputprefix + "jumps_" + p->outputendfix + "_run_" + IntToStr(run) + ".dat",steps);
    MKcurr->saveMaps(p->outputprefix, p->outputendfix, steps);
...
void run::runCurrentRandPos(class params *p)
{

  ESystem *es;
  CGcontrol *esc;
  MKcurrentRandPos *MKcurr;
  string st;

  double  Ex, Ey, Ez, energy,t, Hx, Hy, Hz; // mc,
  int L, D, N, steps, run, runs; //, im, i, s, f

  printf("\n-----Setting up random position system, dynamic algorithm-----\n\n");
...
//  printf("Energy: %le\n",energy);
  ...
  MKcurr = new MKcurrentRandPos(steps);
